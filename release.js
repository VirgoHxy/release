...
class Release {
    ...
    try {
      ...
      if (branch) {
        if (branch == "master") {
          ...
          console.log(
            `只能在 master 分支上执行 ${branch}无法执行 npm run release！`
          );
        }
      } else {
  ...
  buildInit() {
    try {
      ...
      let { buildDir, releaseName, baseDir = desktopDir, versionFlag = false } =
        this.option || {};
      if (!buildDir) {
  ...
  copyReleaseFiles() {
    let diffArr = [];
    let diffStr = this.exec("git diff master origin/master --name-only");
  ...
  copyBackupFiles(diffArr) {
    ...
    this.exec("git reset --hard origin/master");
